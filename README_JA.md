...
| 複数選択とバッチ編集 | 空白ドラッグで**矩形選択**（Shift 追加 / Ctrl 反転、リアルタイムプレビュー、Esc で取消）；右の**バッチ編集パネル**でノードタイプ / 備考 / アイコン / 座標と共通カスタム属性をまとめて編集（値が異なる場合は `—`。タグ条件は「選択中すべてに適用」ボタンで明示的に適用）；**バッチドラッグ**は相対レイアウトを保持し、**バッチ削除**は確認も Undo も 1 回；ツールバーの**整列 / 等間隔分布**はノード中心基準（キャンバスの Y 軸は上向き）；**矢印キー**でグリッド 1 マス分の移動（吸着時はその方向の次のグリッド線へ整列）；**グリッドサイズは設定可能**（`NodeTreeData.gridSize`、既定 20、アセット経由で共有）。 |
