...
| 多选与批量编辑 | 空白拖拽**框选**（Shift 加选 / Ctrl 反选，实时预览、Esc 取消）；右侧**批量编辑面板**统一改节点类型 / 备注 / 图标 / 坐标与共有自定义属性（多值显示 `—`，标签条件需显式「应用到全部选中」）；**批量拖拽**保持相对布局、**批量删除**单次确认与单次 Undo；工具栏**对齐 / 等距分布**（按节点中心，画布 Y 轴向上）；**方向键**按网格步长移动选中节点（吸附开启时先对齐到下一条网格线）；**网格尺寸可配置**（`NodeTreeData.gridSize`，默认 20，随资产共享）。 |
