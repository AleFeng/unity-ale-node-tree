...
- **ランタイムズーム**（1.7.0 以降）：ホイールは**カーソル位置を基点**に、スライダーはビューポート中心を基点にズームします。スケールするのは `nodeTreeRoot` の `localScale` です。範囲は `minZoom`~`maxZoom`（既定 `0.1`~`3`）、既定倍率は `defaultZoom`（`1`）、ホイール 1 ノッチごとに `zoomStepPerScroll`（`0.1`。全域でおよそ 29 ノッチ）を加減します。`zoomSlider` を割り当てればドラッグでズームでき（ウィンドウ側で正規化 `0~1` に強制し、倍率とは**対数マッピング**。既定範囲では 1.0x が 68% の位置）、`zoomValueText` を割り当てれば `zoomValueFormat`（`"{0:0.0}x"`）で現在倍率を表示します。ホイールの入力領域はウィンドウが自動で用意します。詳細は [`UINodeTreeZoomArea`](#uinodetreezoomarea)。
...
**主な API**：`Bind(NodeData, NodeTypeData)` / `Unbind()`（`virtual`。オーバーライド時は必ず `base` を呼ぶこと）、`Show(bool instant = false)` / `Hide(bool instant = false)`（**非仮想**）、読み取り専用の `BoundNode` / `BoundType` / `Rect` / `IsVisible` / `IsRecyclable`。
...
- **オフセットはウィンドウ側に配置**：ポップアップの位置は ノード中心 ＋ `UINodeTreeWindow.infoPanelOffset`（既定 `(0, 120)`。ノードの上）です。`NodeData.position` はそもそもノードの中心なので、サイズに応じた補正は不要です。
...
**説明が空ならポップアップしない**：ウィンドウはプールからインスタンスを取得する**前**にノードの `nodeDesc` を解決し（`ResolveText()`）、空または空白のみの場合は何も表示せず、インスタンスも生成しません —— 中身のない枠は情報を与えず、下のノードを隠すだけだからです。⚠ 判定は **`nodeDesc` のみ**を見ます：ポップアップの文言がそれ以外（デモのように `LocalizeStringEvent` の固定テキスト）から来る場合でも、ノードの `nodeDesc` を入力しないと表示されません。
