...
namespace StoryTreeSystem
{
    ...
    /// </summary>
    public class StoryConditionManager
    {
        private static StoryConditionManager _instance; // 单例实例
        /// <summary>获取全局单例，首次访问时自动创建并注册内置检查器。</summary>
        public static StoryConditionManager Instance => _instance ??= new StoryConditionManager();

        // 已注册的检查器字典，key = IStoryConditionChecker.ConditionType
        private readonly Dictionary<string, IStoryConditionChecker> _checkers
            = new Dictionary<string, IStoryConditionChecker>();

        /// <summary>私有构造函数，自动注册内置条件检查器。</summary>
        private StoryConditionManager()
        {
        ...
        public void Register(IStoryConditionChecker checker)
        {
