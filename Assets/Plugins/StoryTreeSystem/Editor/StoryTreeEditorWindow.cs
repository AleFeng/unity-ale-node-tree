...
namespace StoryTreeSystem.Editor
{
    ...
    /// </summary>
    public class StoryTreeEditorWindow : EditorWindow
    {
        // ── 布局常量 ──
        ...
        #region 基础功能
        // ── 数据 ──
        private ConfigStoryTree _config;                              // 当前编辑的故事树配置资产
        private StoryTreeCanvasState _canvas = new StoryTreeCanvasState(); // 画布交互状态（平移/缩放/选中/拖拽）
        // ── 持久化键（PlayerPrefs） ──
        ...
        /// </summary>
        public static void Open(ConfigStoryTree config)
        {
            var window = OpenWindow();
            if (config)
            {
        ...
        /// 首次创建时居中显示并应用默认尺寸。
        /// </summary>
        private static StoryTreeEditorWindow OpenWindow()
        {
            bool isNew = !HasOpenInstances<StoryTreeEditorWindow>();
            var window = GetWindow<StoryTreeEditorWindow>("Story Tree Editor");
        ...
        private void OnEnable()
        {
            ...
            _canvas.Zoom = Mathf.Clamp(_canvas.Zoom, StoryTreeCanvasState.MinZoom, StoryTreeCanvasState.MaxZoom);
            ...
            {
                _config = AssetDatabase.LoadAssetAtPath<ConfigStoryTree>(path);
        ...
        internal void RebuildLists()
        {
            ...
            _nodeTypeList = new ReorderableList(_config.nodeTypes,
                typeof(StoryNodeTypeData), true, true, true, true);
            ...
            _nodeTypeList.onAddCallback = _ =>
            {
                ...
                _config.nodeTypes.Add(new StoryNodeTypeData { typeName = "新类型" });
                RebuildTypeNameCache();
                MarkDirty();
            };
            ...
            _conditionTypeList = new ReorderableList(_config.conditionTypes,
                typeof(StoryConditionTypeData), true, true, true, true);
            ...
            _conditionTypeList.onAddCallback = _ =>
            {
                ...
                _config.conditionTypes.Add(new StoryConditionTypeData { conditionType = "新条件" });
                RebuildTypeNameCache();
                MarkDirty();
            };
        ...
        private void DrawToolbar()
        {
            using (new EditorGUILayout.HorizontalScope(EditorStyles.toolbar, GUILayout.Height(ToolbarHeight)))
            {
                ...
                var newConfig = (ConfigStoryTree)EditorGUILayout.ObjectField(
                    _config, typeof(ConfigStoryTree), false, GUILayout.Width(200f));
        ...
        private void ZoomBy(float delta)
        {
            _canvas.Zoom = Mathf.Clamp(_canvas.Zoom + delta,
                StoryTreeCanvasState.MinZoom, StoryTreeCanvasState.MaxZoom);
            Repaint();
        }
        #endregion
        #endregion
        ...
        /// </summary>
        private void DrawNodeProperties(StoryNodeData node)
        {
            // 节点切换时清除上一个节点遗留的重名警告
            ...
            if (GUILayout.Button("+ 添加自定义数据"))
                node.customDataList.Add(new StoryNodeCustomData());
            EditorGUILayout.Space(8f);
        ...
        /// </summary>
        private void DrawNodeTypeProperties(StoryNodeTypeData type)
        {
        ...
        private void DrawNodes(Rect localBounds)
        {
            ...
            foreach (var node in _config.nodes)
            {
                ...
                StoryNodeDrawer.DrawNode(node, nodeType, isSelected, isHovered, nodeRect);
        ...
        /// <summary>
        /// 遍历所有节点，为每条父→子关系绘制连线。
        /// 委托给 <see cref="StoryNodeDrawer.DrawAllLineConnections"/> 实现，
        /// 包含 Liang-Barsky 裁剪、颜色高亮等逻辑。
        /// </summary>
        private void DrawAllLineConnections(Rect localBounds)
            => StoryNodeDrawer.DrawAllLineConnections(
        ...
        /// 委托给 <see cref="StoryNodeDrawer.DrawConnectionArrows"/> 实现。
        /// </summary>
        private void DrawConnectionArrows(Rect localBounds)
            => StoryNodeDrawer.DrawConnectionArrows(localBounds, _config, _canvas);
        #endregion
        ...
        private void DrawPreviewConnection()
        {
            ...
            StoryNodeDrawer.DrawLineConnection(clippedFrom, clippedTo, lineStyle,
        ...
        /// </summary>
        private void LayoutSubtree(StoryNodeData node, int depth, bool horizontal, ref float crossCursor)
        {
            if (node == null) return;
