...
| Multi-selection & batch editing | Rubber-band **marquee select** on empty canvas (Shift adds, Ctrl toggles, live preview, Esc cancels); a **batch panel** edits node type / comment / icon / position and the common custom attributes across every selected node (`—` for mixed values; tag conditions require an explicit "apply to all selected"); **batch drag** preserves the relative layout and **batch delete** needs one confirmation and one Undo; toolbar **align / distribute** works on node centers (canvas Y points up); **arrow keys** nudge the selection by one grid cell (landing on the next grid line when snapping is on); the **grid size is configurable** (`NodeTreeData.gridSize`, default 20, shared through the asset). |
